...
class consultas
{
    ...
    public function listarCobradoresAgregados()
    {

        $listadoCobNew = [];
        $sql = "SELECT * FROM s_usuarios  as U INNER JOIN c_historial as H ON U.id_usuario=H.id_usuarioH ORDER BY H.fecha DESC";
        $pdo = $this->MysqlConect->prepare($sql);
        $pdo->execute();

        while ($r = $pdo->fetch(PDO::FETCH_ASSOC)) {
            $listadoCobAg = new UsuarioEHistorial();
            $listadoCobAg->setidH($r['id']);
            $listadoCobAg->setusuario($r['usuario']);
            $listadoCobAg->setclave($r['clave']);
            $listadoCobAg->setfecha($r['fecha']);
            $listadoCobAg->setoficina($r['oficina']);
            $listadoCobAg->setnombre($r['Nombre']);
            array_push($listadoCobNew, $listadoCobAg);
        }
        return $listadoCobNew;
    }
    public function agregar($idusuario)
    {
        ...
        VALUES ('$CVE_COB','$oficina','$nombre','$activado','$liquidacion')";
            ...
            if (oci_num_rows($sql) > 0) {
                //si se realizo cambio se imprime y se envia la alerta tipo javascrip
                //Se ingresa a la tabla de registro de nuevos registros
                date_default_timezone_set('US/Arizona');
                $Hora_de_Registro = date('Y-m-d H:i:s');
                $sql2 = "INSERT INTO c_historial(id_usuarioH,clave,fecha,oficina,Nombre)VALUES(?,?,?,?,?)";
                try {
                    $pdo = $this->MysqlConect->prepare($sql2);
                    $pdo->execute(array($idusuario, $CVE_COB, $Hora_de_Registro, $oficina, $nombre));
                    echo '
